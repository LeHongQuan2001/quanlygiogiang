...
if(isset($_GET['id'])){
$qry = $conn->query("SELECT * FROM giaovien where id= ".$_GET['id']);
...
<div class="container-fluid">
    <form action="" id="manage-giaovien">
...
<script>
    $('#manage-giaovien').on('reset',function(){
        $('#msg').html('')
        $('input:hidden').val('')
    })
    $('#manage-giaovien').submit(function(e){
        e.preventDefault()
        start_load()
        $('#msg').html('')
        $.ajax({
            url:'ajax.php?action=save_giaovien',
