...
if(isset($_GET['id'])){
$qry = $conn->query("SELECT * FROM giaovu where id= ".$_GET['id']);
...
<script>
    $('#manage-giaovu').on('reset',function(){
        $('#msg').html('')
        $('input:hidden').val('')
    })
    $('#manage-giaovu').submit(function(e){
        e.preventDefault()
        start_load()
        $('#msg').html('')
        $.ajax({
            url:'ajax.php?action=save_giaovu',
