...
Class Action {
	...
	function save_giaovien(){
		...
		$check = $this->db->query("SELECT * FROM tbl_users where name ='$id_no' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
	...
	function delete_giaovien(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM tbl_users where id = ".$id);
		if($delete){
			return 1;
		}
	}
	...
	function save_giaovu(){
		...
		$check = $this->db->query("SELECT * FROM giaovu where id_no ='$id_no' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO giaovu set $data");
			$nid=$this->db->insert_id;
		}else{
			$save = $this->db->query("UPDATE giaovu set $data where id = $id");
		}
	...
	function delete_giaovu(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM giaovu where id = ".$id);
		if($delete){
			return 1;
		}
	}
